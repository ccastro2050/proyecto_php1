...
tiene 2: `./api_facturas`, `./front_php`.
...
COPY . .
EXPOSE 8022
CMD ["php", "-S", "0.0.0.0:8022", "index.php"]
```
...
| `EXPOSE 8022` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
| `CMD ["php", "-S", "0.0.0.0:8022", "index.ph…` | **El comando que se ejecuta al encender** el contenedor | Si ese proceso termina, el contenedor se apaga |
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            FRONTPHP["SERVIDOR WEB (front)<br/>contenedor front-php<br/>hostname: front-php · escucha en 8020"]
    ...
    NAV -->|"localhost:8020"| FRONTPHP
    NAV -->|"localhost:8101"| PHPMYADMIN
    APIFACTURAS -->|"mariadb:3306 (DNS de Docker)"| MARIADB
    FRONTPHP -->|"http://api-facturas:8022<br/>al controlador, por el NOMBRE"| APIFACTURAS
    PHPMYADMIN -->|"mariadb:3306 (DNS de Docker)"| MARIADB
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:13326"| MARIADB
```
...
docker network create proyecto_php1_default

# 2. mariadb
docker run -d --name mariadb --network proyecto_php1_default --restart unless-stopped `
  ...
  -v "${PWD}/db/init_phpmyadmin.sql:/docker-entrypoint-initdb.d/init_phpmyadmin.sql:ro" `
  -p 13326:3306 mariadb:11
...
docker run -d --name api-facturas --network proyecto_php1_default --restart unless-stopped `
  -e "DB_DSN=mysql:host=mariadb;port=3306;dbname=bdfacturas_mariadb_local" `
  -e "DB_USUARIO=paradigmas" `
  -e "DB_CLAVE=paradigmas123" `
  -v "${PWD}/api_facturas:/app" `
  -p 8022:8022 api-facturas

# 5. Construir la imagen de front-php y encenderla
docker build -t front-php ./front_php
docker run -d --name front-php --network proyecto_php1_default --restart unless-stopped `
  -e "URL_API=http://api-facturas:8022" `
  -v "${PWD}/front_php:/app" `
  -p 8020:8020 front-php

# 6. phpmyadmin
docker run -d --name phpmyadmin --network proyecto_php1_default --restart unless-stopped `
  -e "PMA_HOST=mariadb" `
  -e "PMA_USER=paradigmas" `
  -e "PMA_PASSWORD=paradigmas123" `
  -p 8101:80 phpmyadmin:latest
...
```yaml
URL_API: http://api-facturas:8022
```
...
**`api-facturas` es un nombre que solo existe dentro de la red de Docker.**
Su navegador corre en Windows, fuera de esa red, y no sabe quién es.

| Desde dónde | Qué dirección sirve | Por qué |
|---|---|---|
| **Su navegador** | `http://localhost:8022` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8022` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8022:8022`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8022`, el front
...
```powershell
docker network inspect proyecto_php1_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_php1` produce
la red `proyecto_php1_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_php1-front-php-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_php1_default has active endpoints (name:"proyecto_php1-front-php-1" …)
```
